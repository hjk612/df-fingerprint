...
def test_datetime_timezone_normalization():
    ...
    df_pst = pd.DataFrame({"t": pd.to_datetime(["2023-12-31 16:00:00"]).tz_localize("US/Pacific")})
...
def test_nan_nat_nulls_normalize():
    df = pd.DataFrame({"x": [np.nan, None, 1], "y": [pd.NaT, pd.Timestamp("2024-01-01", tz="UTC"), None]})
    fp = fingerprint(df)
...
class TestFloatEdgeCases:
    ...
    @pytest.mark.parametrize("special_float,expected_encoding", [
        (float('inf'), ['f', 'inf']),
        (float('-inf'), ['f', '-inf']),
        (-0.0, ['f', '-0.0']),
        (0.0, ['f', '0.0']),
...
class TestTimezoneHandling:
    ...
    def test_naive_vs_aware_same_moment(self):
        ...
        df_aware = pd.DataFrame({"dt": pd.to_datetime(["2024-01-01 12:00:00"], utc=True)})
    ...
    @pytest.mark.parametrize("tz1,tz2,time1,time2", [
        ("UTC", "US/Eastern", "2024-06-01 12:00:00", "2024-06-01 08:00:00"),  # EDT
        ("UTC", "Europe/London", "2024-06-01 12:00:00", "2024-06-01 13:00:00"),  # BST
        ("US/Pacific", "US/Eastern", "2024-06-01 09:00:00", "2024-06-01 12:00:00"),  # Same moment
    ])
...
class TestCategoricalHandling:
    ...
    def test_unsorted_vs_sorted_categories_same_fingerprint(self):
        ...
        # Unsorted categories
        cat1 = pd.Categorical(['red', 'blue', 'green'], categories=['red', 'green', 'blue'])
        df1 = pd.DataFrame({"color": cat1})
        
        # Sorted categories  
        cat2 = pd.Categorical(['red', 'blue', 'green'], categories=['blue', 'green', 'red'])
...
class TestIndexHandling:
    ...
    def test_multiindex_handling(self):
        ...
        idx = pd.MultiIndex.from_tuples([('A', 1), ('B', 2), ('A', 3)], names=['letter', 'number'])
...
class TestMetadataAndVersioning:
    ...
    def test_configuration_in_metadata(self):
        ...
        df = pd.DataFrame({"col": [1, 2, 3]})
        canon = Canonicalizer(sort_columns=False, keep_index=True, float_mode="decimal=1e-6")
        fp = canon.hash(df)
