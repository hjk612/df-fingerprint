...
class TestVectorizedPerformance:
    ...
    def create_large_dataframe(self, n_rows=10000):
        ...
        return pd.DataFrame({
            ...
            'timedelta_col': pd.to_timedelta(np.random.randint(0, 1000, n_rows), unit='D'),
    ...
    def test_vectorized_scaling(self, n_rows):
        ...
        if n_rows <= 5000:
            assert elapsed < 1.0, f"Encoding {n_rows} rows took {elapsed:.2f}s, expected < 1.0s"
    ...
    def test_mixed_types_vectorized(self):
        ...
        df = pd.DataFrame({
            'mixed_col': [
                ...
                b'bytes', pd.Timestamp('2024-01-01'),
                pd.Timedelta('1 day')
            ]
        })
    ...
    def test_datetime_timezones_vectorized(self):
        ...
        df = pd.DataFrame({
            ...
            'dt_tz': pd.to_datetime(['2024-01-01 04:00:00']).tz_localize('US/Pacific'),
        })
